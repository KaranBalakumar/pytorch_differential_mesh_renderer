...
There is an optimized C++ implementation of this renderer available for use. To enable it, first install the kernel via `cd src/mesh_renderer/kernels && python setup.py install`, then change the hardcoded config variable `USE_CPP_RASTERIZER` as described in the [mesh_renderer docs]
(https://github.com/KaranBalakumar/pytorch_differential_mesh_renderer/blob/main/src/mesh_renderer/README.md).
